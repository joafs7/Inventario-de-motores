...
{
    public partial class Motores_de_clientes : Form
    {
        private PrintDocument printDocument = new PrintDocument();
        private int currentRow = 0;
        public Motores_de_clientes()
        {
            InitializeComponent();
        ...
        private void CargarGrilla()
        {
            try
            {
                ...
                dgv_Cliente.DataSource = datosGrilla;
                Conexion.Close();
            }
            catch (Exception ex)
            {
        ...
        private void btn_Guardar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (guardar == "Nuevo")
                {
                    comandoSQL = new SqlCommand("INSERT INTO MotorDeCliente (cliente,marca,hp,rpm,cantidad,cuerpo,volts,hz)" + "VALUES (@Cliente, @Marca,@HP,@RPM,@Cantidad,@Cuerpo,@Volts,@Hz)", Conexion);
                    ...
                    MessageBox.Show("Motores cargados correctamente");
        ...
        private void btn_Modificar_Click(object sender, EventArgs e)
        {
            ...
            {
                foreach (DataGridViewRow row in dgv_Cliente.SelectedRows)
                {
                    codigo = Convert.ToInt32(row.Cells["Codigo"].Value); // Capturamos el ID del cliente seleccionado
                    txt_Cliente.Text = row.Cells["Cliente"].Value.ToString();
        ...
        private void btn_Eliminar_Click(object sender, EventArgs e)
        {
            if (dgv_Cliente.SelectedRows.Count > 0)
            {
                foreach (DataGridViewRow row in dgv_Cliente.SelectedRows)
                {
                    ...
                    Conexion.Open();
                    SqlCommand comandoSQL;
                    comandoSQL = new SqlCommand("DELETE FROM MotorDeCliente WHERE codigo = @Codigo", Conexion);
                    comandoSQL.Parameters.AddWithValue("@Codigo", codigo);
                    comandoSQL.ExecuteNonQuery();
                    Conexion.Close();
                    CargarGrilla();
        ...
        private void Motores_de_clientes_Load(object sender, EventArgs e)
        {
            CargarGrilla();
            LimpiarCampos();
        }
        ...
        private void txt_FiltroCliente_TextChanged(object sender, EventArgs e)
        {
            try
            {
                Conexion.Open();
                ...
                string filtro = txt_FiltroCliente.Text;

                // Crea la consulta SQL con el filtro 
                string consulta = "SELECT * FROM MotorDeCliente WHERE Cliente LIKE @filtro";
                ...
                dgv_Cliente.DataSource = tablaResultados;
        ...
        private void PrintDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            int tableWidth = dgv_Cliente.Columns.Count * colWidth; // Ancho total de la tabla
            ...
            for (int col = 0; col < dgv_Cliente.Columns.Count; col++)
            {
                e.Graphics.DrawString(dgv_Cliente.Columns[col].HeaderText, headerFont, Brushes.Black, x + (col * colWidth) + 5, y + 5);
                e.Graphics.DrawLine(blackPen, x + (col * colWidth), y, x + (col * colWidth), y + rowHeight);
            }
            y += rowHeight;

            // Dibujar filas de datos
            while (currentRow < dgv_Cliente.Rows.Count)
            {
                ...
                if (dgv_Cliente.Rows[currentRow].Cells["Hz"].Value?.ToString() == "60 Hz")
                {
                ...
                for (int col = 0; col < dgv_Cliente.Columns.Count; col++)
                {
                    string cellValue = dgv_Cliente.Rows[currentRow].Cells[col].Value?.ToString() ?? "";
        ...
        private void ExportarPDF(string nombreArchivo)
        {
            ...
            try
            {
                ...
                PdfPTable tabla = new PdfPTable(dgv_Cliente.Columns.Count);
                tabla.WidthPercentage = 100;

                // Encabezados
                foreach (DataGridViewColumn columna in dgv_Cliente.Columns)
                {
                ...
                // Filas
                foreach (DataGridViewRow fila in dgv_Cliente.Rows)
                {
                    if (!fila.IsNewRow)
                    {
